...
int main()
{
    ...
    mtLabel1->setUpdateCallback( new MovingCallback(&viewer)); //link the moving callback to the label
    label1->setText("Label 1", osgText::String::ENCODING_UTF8 );
    label1->setAxisAlignment(osgText::Text::SCREEN);
    label1->setAlignment(osgText::Text::CENTER_BOTTOM); // To center the label on the node // The problem is that the movingCallback with the line is not working properly anymore.
    ...
    //label1->setAutoRotateToScreen(false);  // To make the label stay "on" the map : + alors remove the movingcallback.
    label1->setCharacterSize(3);
    label1->setDrawMode(3); // To add a pseudo bounding box (on the text, not on the object label)
    //label1->setDrawMode(5);   // To add a backgrouund !
    ...
    mtLabel2->setUpdateCallback( new MovingCallback(&viewer));
    ...
    // Create picker
    ref_ptr<PickHandler> picker = new PickHandler(mtLabel1.get());
